# COPRegion Install Appliance A Configuration
# stdlib
import json
import ipaddress
import subprocess
...
def build(win):
    ...
    updated_config['podnet_a_enabled'] = True
    ...
    try:
        subprocess.run(
            'curl -s https://raw.githubusercontent.com/CloudCIX/pod_yaml/master/copregion/nginx1.conf.template -o /etc/cloudcix/docker/templates/nginx1/default.conf.template > /dev/null 2>&1',
            shell=True,
            check=True,
        )
        subprocess.run(
            'curl -s https://raw.githubusercontent.com/CloudCIX/pod_yaml/master/copregion/nginx2.conf.template -o /etc/cloudcix/docker/templates/nginx2/default.conf.template > /dev/null 2>&1',
            shell=True,
            check=True,
        )
