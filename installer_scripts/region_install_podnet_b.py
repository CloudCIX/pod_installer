...
def build(win):
    ...
    # sort ipaddresses
    recovery_ip = f'100.64.{config_data["pod_number"]}.253'
    configured, error = net.build(
        host='localhost',
    ...
    firewall_rules = [
        ...
        {'order': 3151, 'version': '4', 'iiface': '', 'oiface': 'any', 'protocol': 'any', 'action': 'accept', 'log': True, 'source': ['127.0.0.0/8', config_data['ipv4_link_cpe'], f'{pms_ips[2]}', recovery_ip], 'destination': ['any'], 'port': []},
