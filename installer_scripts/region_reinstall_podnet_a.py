# Region Re-Install PodNet A Configuration
# stdlib
import ipaddress
import json
import os
...
def build(win):
    ...
    firewall_rules = [
        # 3.1.1 Inbound IPv4
        # a: "lo" icmp accept
        {'order': 3111, 'version': '4', 'iiface': 'lo', 'oiface': '', 'protocol': 'icmp', 'action': 'accept', 'log': False, 'source': ['127.0.0.1', config_data['ipv4_link_cpe'], f'{pms_ips[0]}', f'{pms_ips[1]}'], 'destination': ['127.0.0.1', config_data['ipv4_link_cpe'], f'{pms_ips[0]}', f'{pms_ips[1]}'], 'port': []},
        # b: "lo" dns accept
        ...
        {'order': 3115, 'version': '4', 'iiface': 'mgmt0', 'oiface': '', 'protocol': 'icmp', 'action': 'accept', 'log': False, 'source': [config_data['primary_ipv4_subnet'], config_data['ipv4_link_pe']] + [asgn.strip() for asgn in config_data['pat_region_assignments'].split(',')], 'destination': [f'{pms_ips[0]}', f'{pms_ips[1]}', config_data['ipv4_link_cpe']], 'port': []},
        ...
        {'order': 3122, 'version': '6', 'iiface': 'public0', 'oiface': '', 'protocol': 'icmp', 'action': 'accept', 'log': False, 'source': [config_data['ipv6_link_pe'], config_data['pat_ipv6_subnet'], 'fe80::/10'], 'destination': [config_data['ipv6_link_cpe'], 'fe80::/10'], 'port': []},
        # j: Ping Accept on Mgmt interface
        {'order': 3123, 'version': '6', 'iiface': 'mgmt0', 'oiface': '', 'protocol': 'icmp', 'action': 'accept', 'log': False, 'source': [config_data['ipv6_link_pe'], config_data['pat_ipv6_subnet'], f'{mgmt_ipv6_3hex}:d0c6::/64', f'{mgmt_ipv6_3hex}::/64', 'fe80::/10'], 'destination': [config_data['ipv6_link_cpe'], f'{mgmt_ipv6_3hex}::10:0:1', f'{mgmt_ipv6_3hex}::10:0:2', 'ff00::/8', 'fe80::/10'], 'port': []},
        # k: SSH to Mgmt Interface by Robot
        {'order': 3124, 'version': '6', 'iiface': 'mgmt0', 'oiface': '', 'protocol': 'tcp', 'action': 'accept','log': False, 'source': [f'{mgmt_ipv6_3hex}:d0c6::6001:1', f'{mgmt_ipv6_3hex}:d0c6::6001:2', f'{mgmt_ipv6_3hex}::6000:1'], 'destination': [f'{mgmt_ipv6_3hex}::10:0:2'], 'port': ['22']},
        ...
        # b: PUBLIC to MGMT : COP nginx(pms4) and portal(pms5) 443 Accept: N/A
        # c: MGMT to PUBLIC: Outbound http
        {'order': 3132 'version': '4', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'tcp', 'action': 'accept', 'log': True, 'source': [config_data['primary_ipv4_subnet']], 'destination': ['any'], 'port': ['80']},
        # d: MGMT to PUBLIC: Outbound https
        {'order': 3133, 'version': '4', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'tcp', 'action': 'accept', 'log': True, 'source': [config_data['primary_ipv4_subnet']], 'destination': ['any'], 'port': ['443']},
        # e: MGMT to PUBLIC: Outbound ntp
        {'order': 3134, 'version': '4', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'udp', 'action': 'accept', 'log': True, 'source': [config_data['primary_ipv4_subnet']], 'destination': ['any'], 'port': ['123']},
        # f: MGMT to PUBLIC: Outbound dns
        {'order': 3135, 'version': '4', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'udp', 'action': 'accept', 'log': True, 'source': [config_data['primary_ipv4_subnet']], 'destination': ['any'], 'port': ['53']},
        # g: PUBLIC to and from SUBNET BRIDGES: All inbound to projects are via Subnet Bridge and its Interfaces
        {'order': 3136, 'version': '4', 'iiface': '!={mgmt0, ha.44, private0, inter0}', 'oiface': '!={mgmt0, ha.44, private0, inter0}', 'protocol': 'any', 'action': 'accept', 'log': False, 'source': ['any'], 'destination': ['any'], 'port': []},
        ...
        # i: PUBLIC to MGMT: COP nginx and portal 443 Accept: N/A
        # j: MGMT to PUBLIC: Outbound http
        {'order': 3142, 'version': '6', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'tcp', 'action': 'accept', 'log': True, 'source': [f'{mgmt_ipv6_3hex}:d0c6::/64', f'{mgmt_ipv6_3hex}::/64'], 'destination': ['any'], 'port': ['80']},
        # k: MGMT to PUBLIC: Outbound https
        {'order': 3143, 'version': '6', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'tcp', 'action': 'accept', 'log': True, 'source': [f'{mgmt_ipv6_3hex}:d0c6::/64', f'{mgmt_ipv6_3hex}::/64'], 'destination': ['any'], 'port': ['443']},
        # l: MGMT to PUBLIC: Outbound ntp
        {'order': 3144, 'version': '6', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'udp', 'action': 'accept', 'log': True, 'source': [f'{mgmt_ipv6_3hex}:d0c6::/64', f'{mgmt_ipv6_3hex}::/64'], 'destination': ['any'], 'port': ['123']},
        # m: MGMT to PUBLIC: Outbound dns
        {'order': 3145, 'version': '6', 'iiface': 'mgmt0', 'oiface': 'public0', 'protocol': 'udp', 'action': 'accept', 'log': True, 'source': [f'{mgmt_ipv6_3hex}:d0c6::/64', f'{mgmt_ipv6_3hex}::/64'], 'destination': ['any'], 'port': ['53']},
        # n: PUBLIC to and from SUBNET BRIDGES: All inbound to projects are via Subnet Bridge and its Interfaces
        {'order': 3146, 'version': '6', 'iiface': '!={mgmt0, ha.44, private0, inter0}', 'oiface': '!={mgmt0, ha.44, private0, inter0}', 'protocol': 'any', 'action': 'accept', 'log': False, 'source': ['any'], 'destination': ['any'], 'port': []},
        # PUBLIC to HA: Inbound Block From Public to HA: Since default rules are blocked, no need this
        # HA to PUBLIC: Outbound Block From HA to Public: Since default rules are blocked, no need this
        # PUBLIC to PRIVATE: No traffic between public0 to private0
        # PRIVATE to PUBLIC: No traffic between private0 to public0
        ...
        # b: Allow all
        {'order': 3152, 'version': '4', 'iiface': '', 'oiface': 'any', 'protocol': 'any', 'action': 'accept', 'log': True, 'source': ['127.0.0.0/8', config_data['ipv4_link_cpe'], f'{pms_ips[0]}', f'{pms_ips[1]}', ha44_ip], 'destination': ['any'], 'port': []},
        # c: Allow to 443, robosoc blocklist update
        {'order': 3153, 'version': '4', 'iiface': '', 'oiface': 'any', 'protocol': 'tcp', 'action': 'accept', 'log': False, 'source': ['any'], 'destination': ['any'], 'port': ['443']},

        # 3.1.6 Outbound IPv6
        # d: Allow to Logstash
        {'order': 3161, 'version': '6', 'iiface': '', 'oiface': 'any', 'protocol': 'udp', 'action': 'accept', 'log': False, 'source': [config_data['ipv6_link_cpe'], f'{mgmt_ipv6_3hex}::10:0:2'], 'destination': ['2a02:2078:14:1003::39'], 'port': ['10515']},
        # e: Allow all
        {'order': 3162, 'version': '6', 'iiface': '', 'oiface': 'any', 'protocol': 'any', 'action': 'accept', 'log': True, 'source': [config_data['ipv6_link_cpe'], f'{mgmt_ipv6_3hex}::10:0:2', f'{mgmt_ipv6_3hex}::10:0:1', 'fe80::/10'], 'destination': ['any'], 'port': []},
        # f: Allow to 443, robosoc blocklist update
        {'order': 3163, 'version': '6', 'iiface': '', 'oiface': 'any', 'protocol': 'tcp', 'action': 'accept', 'log': False, 'source': ['any'], 'destination': ['any'], 'port': ['443']},
    ]
